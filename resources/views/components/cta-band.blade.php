...
<section {{ $attributes->merge(['class' => 'relative overflow-hidden bg-night-950 text-white']) }}>
    ...
    <div class="container-page relative py-16 md:py-20">
        <div class="flex flex-col gap-10 lg:flex-row lg:items-center lg:justify-between">
            <div class="max-w-xl">
                <h2 class="text-2xl font-bold md:text-3xl">{{ $title ?? content('common.cta_quote_title') }}</h2>
                <p class="mt-4 text-base leading-relaxed text-night-300">{{ $body ?? content('common.cta_quote_body') }}</p>
            </div>

            <div class="flex flex-col gap-3 sm:flex-row lg:shrink-0">
                <x-button :href="route('quote')" variant="accent" size="lg">
                    {{ content('common.cta_quote_action') }}
                </x-button>

                <x-button :href="route('contact')" variant="inverse" size="lg">
                    {{ content('common.cta_contact_action') }}
                </x-button>
            </div>
        </div>
    </div>
